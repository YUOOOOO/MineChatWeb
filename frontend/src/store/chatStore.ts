    ...
    (set, get) => ({
      ...
      generateConversationTitle: async (conversationId: string) => {
        try {
          ...
          // 导入标题生成服务
          const { titleGenerationService } = await import(
            '@/services/titleGenerationService'
          )

          // 检查是否应该生成标题（恰好2条消息）
          if (
            !titleGenerationService.shouldGenerateTitle(conversation.messages)
          ) {
          ...
          if (settings.apiKeyType === 'builtin') {
            apiKey = settings.builtinModelAccessKey
          } else {
            apiKey =
              settings.apiKeys?.[
          ...
          // 使用打字机效果更新标题
          await get().updateConversationTitleAnimated(
            conversationId,
            generatedTitle
          )
        } catch (error: any) {
      ...
      sendMessage: async (
        ...
        const { useSettingsStore } = await import('./settingsStore')
        const { modelConfigService } = await import(
        ...
        if (settings.apiKeyType === 'builtin') {
          ...
          apiKey = settings.builtinModelAccessKey
          if (!apiKey) {
            throw new Error('请先在设置中配置内置模型访问密钥')
          }
        } else {
          // 使用自定义API密钥
        ...
        const effectiveThinkingMode = isGPT5Model
          ? true
        ...
        // 检查模型是否支持流式输出
        const supportsStreaming = await modelConfigService.supportsStreaming(
        ...
        if (supportsStreaming) {
          // 使用流式输出（包括thinking模式）
          await get()._sendMessageWithStreaming(
            content,
      ...
      _sendMessageWithStreaming: async (
        content: string,
        targetConversationId: string,
        settings: any,
        apiKey: string,
        ...
        try {
          ...
          const normalizeImageGeneration = (
            raw: any
            ...
            const extractBase64 = (): string | undefined => {
              ...
              if (
                typeof raw.image_base64_png === 'string' &&
                raw.image_base64_png.trim()
              ) {
              ...
              if (Array.isArray(raw.output) && raw.output.length > 0) {
                ...
                if (
                  first &&
                  typeof first.image_base64 === 'string' &&
                  first.image_base64.trim()
                ) {
            ...
            const imageId =
              typeof raw.id === 'string' && raw.id.trim()
                ? raw.id
                : `image-${Date.now()}-${
                    Object.keys(imageGenerationMap).length
                  }`
          ...
          const attemptWebSocketConnection = async (
            ...
            try {
              ...
              // 动态导入工具配置函数
              const { supportsNativeWebSearch, buildWebSearchToolConfig } =
                await import('../lib/webSearchUtils')
              ...
              const request: any = {
                ...
                thinking_mode:
                  ...
                  settings.chatModel?.includes('gpt-5') ||
                  false,
              ...
              if (
                settings.chatProvider === 'openai_compatible' &&
                settings.openaiCompatibleConfig?.baseUrl
              ) {
              ...
              if (tools && tools.length > 0) {
                ...
                if (tools.some((tool) => tool.id === 'search')) {
                  const useNativeSearch = supportsNativeWebSearch(
                    settings.chatProvider,
                    settings.chatModel
                  )
                ...
                // 添加MCP服务器和函数调用工具
                const mcpAndFunctionTools = tools.filter(
                  (tool) =>
              ...
              ws.onmessage = (event) => {
                try {
                  ...
                  if (
                    chunk.choices &&
                    chunk.choices[0]?.delta?.image_generation
                  ) {
              ...
              ws.onerror = (error) => {
                ...
                if (attempt < get().wsMaxReconnectAttempts) {
                  console.log(
                    `WebSocket连接失败，尝试重连 (${attempt + 1}/${
                      get().wsMaxReconnectAttempts
                    })`
                  )
                  setTimeout(() => {
              ...
              ws.onclose = (event) => {
                ...
                if (!event.wasClean && attempt < get().wsMaxReconnectAttempts) {
                  ...
                  !event.wasClean &&
                  attempt >= get().wsMaxReconnectAttempts
                ) {
              ...
              console.error(
                `WebSocket连接失败 (attempt ${attempt + 1}):`,
                error
              )
              if (attempt < get().wsMaxReconnectAttempts) {
                setTimeout(() => {
      ...
      _sendMessageNormal: async (
        ...
        try {
          ...
          if (tools && tools.length > 0) {
            ...
            // 添加MCP服务器和函数调用工具
            const mcpAndFunctionTools = tools.filter(
              (tool) => tool.id !== 'search' && tool.id !== 'image-generation'
            )
            requestTools.push(...mcpAndFunctionTools)
          ...
          const imageGenerations =
            data.choices[0]?.message?.image_generations || []

          // 提取引用和来源信息
          const { extractCitations, extractSearchSources } = await import(
            '../lib/webSearchUtils'
          )
      ...
      regenerateLastMessage: async () => {
        ...
        // 移除最后一条助手消息
        const messagesWithoutLast = currentConversation.messages.slice(
          0,
          lastAssistantIndex
        )
